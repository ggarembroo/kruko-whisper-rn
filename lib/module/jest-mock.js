...
global.whisperInitContext = jest.fn(async contextId => ({
...
global.whisperTranscribeData = jest.fn(async (_contextId, options) => {
  var _options$onProgress;
  (_options$onProgress = options.onProgress) === null || _options$onProgress === void 0 ? void 0 : _options$onProgress.call(options, 100);
  return transcribeResult;
});
global.whisperAbortTranscribe = jest.fn(async () => undefined);
global.whisperBench = jest.fn(async () => JSON.stringify(['NEON', 1, 1, 1, 1, 1]));
global.parakeetInitContext = jest.fn(async contextId => ({
  contextId,
  gpu: false,
...
global.whisperInitVadContext = jest.fn(async contextId => ({
  contextId,
  gpu: false,
