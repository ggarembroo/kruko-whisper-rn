 ...
 */
export function toPcm16ArrayBuffer(
  ...
  if (buffer.byteLength % 2 !== 0) {
    throw new Error(
      'kruko-whisper-rn: expected signed 16-bit PCM, but the buffer has an odd ' +
        `byte length (${buffer.byteLength}). If this came from a Float32Array, ` +
        'pass the Float32Array itself rather than its .buffer — it will be ' +
        'converted for you.'
    )
  }
 ...
 */
export function pcmPeak(pcm: ArrayBuffer): number {
